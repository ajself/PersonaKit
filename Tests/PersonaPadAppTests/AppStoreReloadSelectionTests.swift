...
struct AppStoreReloadSelectionTests {
  ...
  func reloadMapsPackLocationsForDirectoryAndFilePacks() throws {
    let homeDirectory = URL(fileURLWithPath: "/tmp/personapad-tests-home", isDirectory: true)
    let packsRoot = PersonaPadStoragePaths.standard(homeDirectory: homeDirectory).packs
    ...
    fileSystem.write(
      packJSON(
        packID: "com.personapad.directory-pack",
    ...
    fileSystem.write(
      packJSON(
        packID: "com.personapad.file-pack",
        packName: "File Pack",

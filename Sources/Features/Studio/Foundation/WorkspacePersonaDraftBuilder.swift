...
/// Normalizes, validates, and serializes persona drafts for Studio create flows.
public struct WorkspacePersonaDraftBuilder: Sendable {
  ...
  public func buildRawJSON(
    ...
    let normalized = normalizedDraft(draft)
    let document = WorkspacePersonaDocument(
      id: normalized.id,
      version: "1.0",
    ...
    guard var rawJSON = String(data: data, encoding: .utf8) else {
      throw WorkspaceSnapshotBuildError(
        message: "Failed to encode persona JSON."
      )
    }
...
private struct WorkspacePersonaDocument: Codable, Sendable {
  let id: String
  let version: String
  let name: String
  let summary: String

    ...
    static func validate(root: URL, fileManager: FileManager = .default) -> [SchemaValidationError] {
        ...
        for mapping in mappings {
            ...
            let sortedFiles = files
                .filter { $0.lastPathComponent.hasSuffix(mapping.suffix) }
            ...
            for fileURL in sortedFiles {
                ...
                schemaNode.validate(value: json, path: "", relativePath: relativePath, schemaName: mapping.schemaName, errors: &fileErrors)
...
private indirect enum SchemaNode {
    ...
    func validate(
        ...
        case let .object(properties, required):
            guard let object = value as? [String: Any] else {
                errors.append(typeError(relativePath: relativePath, schemaName: schemaName, path: path, expected: "object", actual: value))
                return
            }
        ...
        case let .array(items):
            guard let array = value as? [Any] else {
                errors.append(typeError(relativePath: relativePath, schemaName: schemaName, path: path, expected: "array", actual: value))
                return
            }
        ...
        case .string:
            ...
            errors.append(typeError(relativePath: relativePath, schemaName: schemaName, path: path, expected: "string", actual: value))
        case .boolean:
            if value is Bool {
                return
            }
            errors.append(typeError(relativePath: relativePath, schemaName: schemaName, path: path, expected: "boolean", actual: value))
        case .number:
            if let number = value as? NSNumber, !isBoolNumber(number) {
                return
            }
            errors.append(typeError(relativePath: relativePath, schemaName: schemaName, path: path, expected: "number", actual: value))
        case .integer:
            if let number = value as? NSNumber, !isBoolNumber(number) {
            ...
            errors.append(typeError(relativePath: relativePath, schemaName: schemaName, path: path, expected: "integer", actual: value))
        }
    }
    ...
    private func typeError(relativePath: String, schemaName: String, path: String, expected: String, actual: Any) -> SchemaValidationError {

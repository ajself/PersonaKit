...
4. Update closeout records:
   - `Docs/PersonaKit/Development/logs/gardening-events.jsonl` and refresh the generated
     `Docs/PersonaKit/Development/pack-gardener-log.md`
   - `Docs/PersonaKit/Development/logs/partner-context-events.jsonl` when partner behavior or
     guardrails change, and refresh the generated
     `Docs/PersonaKit/Development/partner-context-log.md`
   - the active execution plan or equivalent work-tracking document
     (for Orbit, start with `Docs/Orbit/Planning/Orbit-Execution-Plan.md`)
5. Append one event to `Docs/PersonaKit/Development/logs/gardening-events.jsonl`.
6. Run verification:
...
1. Maintenance records and the active execution plan reflect current open work
   only.
2. Durable operations docs remain in `Docs/PersonaKit/Development/`.

...
| PersonaKit scope unavailable | workspace cannot resolve a readable PersonaKit scope for contract grounding | block send before publication | no new durable thread state | explicit contract-unavailable error outside the thread | test proves resolution fails cleanly |

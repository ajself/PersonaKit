...
| Unknown collaborator target | direct address points to no participant in the workspace | block activation | optional system-event only, no collaborator response | explicit target-not-found message | test proves no fallback collaborator reply |
...
| Unresolved `ProdDoc` mapping | first-checkpoint collaborator model still has unresolved alias or persona mapping | mark blocked for identity-sensitive activation | no successful activation record for that collaborator | explicit blocked state | test proves identity-sensitive flows do not proceed |
| Missing directive | contract resolution returns no directive | block activation | failed activation attempt may be logged, but no collaborator response | explicit activation-unavailable state | test proves no generic response is substituted |
| Unauthorized skill posture | the required response path would rely on disallowed skill use | block activation | failed activation attempt may be logged | explicit authorization-blocked state | test proves stop before response publication |
...
| Activation persistence failure | response path cannot durably write activation state | fail closed | no completed response record presented as attributable | explicit error or blocked state | test proves trace-less collaborator output is disallowed |

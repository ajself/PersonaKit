...
| Collaborator identity mapping | `architectural-editor` | model review note | each visible AI collaborator has a stable identity anchor | unresolved alias or blended identity |
...
| Contract snapshot inspectability | `architectural-editor` | golden example plus review | directive, kits or equivalent, skill posture, and stop-point posture are inspectable | response trace cannot explain why it was allowed |
...
| Authorization blocking | `studio-coverage-architect` | deterministic failure tests | unauthorized response path is blocked | response proceeds anyway |
